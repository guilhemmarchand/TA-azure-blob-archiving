...
filehandler = logging.FileHandler(
    splunkhome + "/var/log/splunk/azure2blob_azgetsize.log", "a"
)
            ...
            for subrecord in chunk:
                ...
                #
                    ...
                    try:
                        ...
                        logging.info(
                            'blob_list="{}"'.format(json.dumps(blob_list, indent=1))
                        )
                        size_bytes = "unknown"

                        for blob in blob_list:
                            blob_client = blob_service_client.get_blob_client(
                                container=AZ_BLOB_CONTAINER, blob=blob
                            )
                            logging.info(
                                'retrieving size for blob_name="{}"'.format(blob)
                            )
                            properties = blob_client.get_blob_properties()
                            logging.debug(
                                'blob="{}" properties="{}"'.format(
                                    str(subrecord_blob_file), str(properties)
                                )
                            )
                        ...
                        # log
                        logging.info(
                            'blob="{}" size_bytes="{}"'.format(
                                str(subrecord_blob_file), str(size_bytes)
                            )
                        )

                    except Exception as e:
                        logging.error(
                            'Error while getting the blob file size with exception="{}"'.format(
                                str(e)
                            )
                        )
                    ...
                    if self.update_table == "true":
                        ...
                        try:
                            ...
                            table_record_update = {
                                ...
                                "status": table_record.get("status"),
                            }
