...
# set logging
filehandler = logging.FileHandler(splunkhome + "/var/log/splunk/azure2blob_azgetsize.log", 'a')
...
class AzPurgeBlob(StreamingCommand):
    ...
    mode = Option(
        ...
        **Description:** The run mode of the command, by safety it defaults to simulate, valid options are: simulate|live.''',
        require=True, validate=validators.Match("mode", r"^(simulate|live)$"))
    ...
    update_table = Option(
        ...
        **Description:** Specify if the table entity status field will be updated, requires table fields to be provided, defaults to true''',
        require=True, default="true", validate=validators.Match("mode", r"^(true|false)$"))
    ...
    def stream(self, records):
        ...
        if self.update_table == 'true':
            if not self.field_table or not self.field_partitionkey or not self.field_row_key:
                logging.error("update_table is set to true, but the table metadata fields have not been provided")
                sys.exit(1)
            ...
            for subrecord in chunk:
                ...
                #
                    ...
                    try:
                        ...
                        blob_service_client = BlobServiceClient.from_connection_string(conn_str=AZ_BLOB_CONNECTION_STRING)
                        blob_list = [subrecord_blob_file]

                        logging.info("blob_list=\"{}\"".format(json.dumps(blob_list, indent=1)))
                        size_bytes = "unknown"

                        for blob in blob_list:
                            blob_client = blob_service_client.get_blob_client(container=AZ_BLOB_CONTAINER, blob=blob)
                            logging.info("retrieving size for blob_name=\"{}\"".format(blob))
                            properties = blob_client.get_blob_properties()
                            logging.debug("blob=\"{}\" properties=\"{}\"".format(str(subrecord_blob_file), str(properties)))
                            ...
                            logging.info("new record=\"{}\"".format(new_subrecord))

                        # increment
                        success_count+=1

                        # log
                        logging.info("blob=\"{}\" size_bytes=\"{}\"".format(str(subrecord_blob_file), str(size_bytes)))

                    except Exception as e:
                        logging.error("Error while getting the blob file size with exception=\"{}\"".format(str(e)))

                    # if the blob size was successfully received and the option is enabled, attempt to update the table
                    if self.update_table =='true':
                        ...
                        try:
                            ...
                            table_record_update = {
                                ...
                                'epoch_start': table_record.get('epoch_start'), 'epoch_end': table_record.get('epoch_end'), 'size_bytes': str(size_bytes),
                                'indexname': table_record.get('indexname'), 'clustered_flag': table_record.get('clustered_flag'), 'status': table_record.get('status')
                            }
