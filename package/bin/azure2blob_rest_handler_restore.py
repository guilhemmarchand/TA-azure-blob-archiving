...
filehandler = logging.FileHandler(
    splunkhome + "/var/log/splunk/azure2blob_rest_handler_restore.log", "a"
)
...
if os.path.exists(
    ...
    os.path.join(splunkhome, "etc", "peer-apps", "TA-azure-blob-archiving", "lib")
):
    sys.path.append(
        os.path.join(splunkhome, "etc", "peer-apps", "TA-azure-blob-archiving", "lib")
    )
...
class AzRestore_v1(azure2blob_rest_handler.RESTHandler):
    ...
    def post_ta_azure_blob_archiving_v1_restore(self, request_info, **kwargs):
        ...
        if describe:
            ...
            if (
                str(AZ_BLOB_CONNECTION_STRING)
                == "connection_string_to_the_blob_storage"
            ):
                logging.error(
            ...
            try:
                ...
                logging.info(
                    'Attempting to download blob file="{}" to target_file="{}"'.format(
                        str(blob_name), str(target_file)
                    )
                )
                ...
                if os.path.exists(target_file):
                    ...
                    logging.info(
                        'blob file successfully downloaded to target_file="{}", size_bytes="{}"'.format(
                            str(target_file), str(size_bytes)
                        )
                    )
                ...
                try:
                    ...
                    directory_onfilesystem = (
                        str(target_directory) + "/" + str(archive_top_directory)
                    )
                    for item in tar:
                    ...
                    if splunk_rebuild:
                        ...
                        process = subprocess.Popen(
                            [splunk_bin, "rebuild", str(directory_onfilesystem)],
                    ...
                    logging.error(
                        'failed to extract the file target_file="{}" in target_directory="{}" with exception="{}"'.format(
                            target_file, target_directory, str(e)
                        )
                    )
                    # Remove the tgz
                    if os.path.exists(target_file):
