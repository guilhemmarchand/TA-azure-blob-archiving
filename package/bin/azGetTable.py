...
filehandler = logging.FileHandler(
    splunkhome + "/var/log/splunk/azure2blob_azgettable.log", "a"
)
...
class AzGetTable(GeneratingCommand):
    ...
    def generate(self, **kwargs):
        ...
        if self:
            ...
            tasks = table_service.query_entities(self.target_table, filter=filterStr)
            for task in tasks:
                record = {
                    "PartitionKey": task.get("PartitionKey"),
                    "RowKey": task.get("RowKey"),
                    "blob_name": task.get("blob_name"),
                    "bucket_id": task.get("bucket_id"),
                    "original_bucket_name": task.get("original_bucket_name"),
                    "original_peer_name": task.get("original_peer_name"),
                    "original_peer_guid": task.get("original_peer_guid"),
                    "epoch_start": task.get("epoch_start"),
                    "epoch_end": task.get("epoch_end"),
                    "size_bytes": task.get("size_bytes"),
                    "indexname": task.get("indexname"),
                    "clustered_flag": task.get("clustered_flag"),
                    "status": task.get("status"),
                }

                # yield
                data = {
                    "_time": task.get("epoch_end"),
                    "_raw": json.dumps(record, indent=1),
                }
                yield data
            ...
            data = {
                "_time": time.time(),
                "_raw": '{"response": "' + "Error: bad request}",
            }
            yield data
