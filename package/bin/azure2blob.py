...
class Azure2BlobRestHandler(GeneratingCommand):
    ...
    def generate(self, **kwargs):
        ...
        if (self.url and re.search(r"^\/services\/ta_azure_blob_archiving\/v\d*", self.url)) and self.mode in ("get", "post", "delete"):
            ...
            # yield
            data = {'_time': time.time(), '_raw': "{\"response\": \"" + "Error: bad request, URL must match /services/ta_azure_blob_archiving/v1/<endpoint> and accepted HTTP modes are get / post / delete\"}"}
            yield data
